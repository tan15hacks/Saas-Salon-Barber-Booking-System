...
export function AdminWorkspaceSmart() {
      ...
      <div className="space-y-8">
        <div className={panelClass}>
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-7 text-espresso/60">Working hours are {formatTime(workHours.open)} – {formatTime(workHours.close)}. Confirmed bookings automatically remove unavailable staff time slots.</p>
          </div>
  ...
  function renderAppointments() {
    ...
    const manualBookingEnd = minutesToTime(timeToMinutes(bookingTime) + getServiceDuration(bookingServiceId));
    return (
      <div className="space-y-8">
        <form onSubmit={addManualBooking} className={panelClass}>
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><p className="text-sm font-black uppercase tracking-[0.22em] text-rosewood/70">Quick create</p><h3 className="mt-2 text-2xl font-black">Add booking manually</h3><p className="mt-2 text-sm text-espresso/60">Working hours: {formatTime(workHours.open)} – {formatTime(workHours.close)}. Confirmed bookings block overlapping slots.</p></div><span className="w-fit rounded-full bg-blush/60 px-4 py-2 text-sm font-black text-rosewood">Auto-saved</span></div>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            ...
            <select value={bookingTime} onChange={(event) => setBookingTime(event.target.value)} className="input-field">{selectedStaffSlots.map((slot) => <option key={slot} value={slot}>{formatTime(slot)}</option>)}</select>
          </div>
