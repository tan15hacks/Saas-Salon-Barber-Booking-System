  ...
  function renderBookingForm() {
    return (
      <form onSubmit={addManualBooking} className="rounded-[2rem] border border-rosewood/10 bg-white/80 p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-espresso/60">For walk-ins, phone calls, or Messenger bookings.</p>
      ...
      <div className="overflow-hidden rounded-[2rem] border border-rosewood/10 bg-white/80 shadow-sm">
        <div className="flex flex-col gap-3 border-b border-rosewood/10 p-6 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-espresso/60">Confirm, complete, cancel, or mark no-show. This is front-end state until the database is added.</p>
          </div>
    ...
    return (
      <div className="space-y-6">
        <div className="rounded-[2rem] border border-rosewood/10 bg-white/70 p-6 shadow-sm">
          ...
          <p className="mt-3 max-w-3xl leading-7 text-espresso/65">Protected by a mock owner PIN, with bookings, analytics, reports, services, staff, customers, and business settings ready for demo use.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-4">
          {[
            ["Bookings", todayBookings.toString(), "Total records"],
            ["Pending", pendingBookings.toString(), "Needs action"],
  ...
  function renderCalendar() {
    return (
      <div className="rounded-[2rem] border border-rosewood/10 bg-white/80 p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-espresso/60">A simple calendar-style view grouped by staff.</p>
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {staffList.map((staff) => (
            <div key={staff.id} className="rounded-[1.5rem] bg-cream p-5">
              ...
              <div className="mt-5 space-y-3">
                ...
                {!bookingList.some((booking) => booking.staffId === staff.id) ? <p className="rounded-2xl bg-white/70 p-4 text-sm text-espresso/60">No bookings yet.</p> : null}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-[2rem] border border-rosewood/10 bg-white/80 p-6 shadow-sm">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {customers.map((customer) => (
            <div key={customer.customerPhone} className="rounded-[1.5rem] bg-cream p-5"><p className="font-black">{customer.customerName}</p><p className="mt-1 text-sm text-espresso/60">{customer.customerPhone}</p><p className="mt-3 text-sm font-bold text-rosewood">Latest: {getServiceName(customer.serviceId)}</p></div>
          ))}
        </div>
      </div>
    );
      ...
      <div className="salon-container">
        <div className="grid gap-6 lg:grid-cols-[270px_1fr]">
          <aside className="h-fit rounded-[2rem] bg-espresso p-5 text-white lg:sticky lg:top-28">
            ...
            <div className="mt-6 grid grid-cols-3 gap-2 text-center text-xs md:grid-cols-3 lg:grid-cols-1 lg:text-left"><div className="rounded-2xl bg-white/10 p-3"><b>{pendingBookings}</b> pending</div><div className="rounded-2xl bg-white/10 p-3"><b>{completedBookings}</b> done</div><div className="rounded-2xl bg-white/10 p-3"><b>{staffList.length}</b> staff</div></div>
