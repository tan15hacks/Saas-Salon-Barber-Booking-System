...
function AvailabilityBadge({ full }: { full: boolean }) {
  return full ? (
    <span className="rounded-full bg-red-100 px-3 py-1 text-xs font-black text-red-700">Fully booked today</span>
  ) : (
    <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">Available today</span>
  );
}
...
export function AdminWorkspaceFixed() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setStaffAvailability(saved.staffAvailability ?? availability);
    } else {
  ...
  useEffect(() => {
    if (!isLoaded) return;
    const state: SavedWorkspaceState = { bookings, services, staff, shopName, shopPhone, shopHours, staffAvailability };
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
    window.localStorage.setItem(STAFF_AVAILABILITY_KEY, JSON.stringify(staffAvailability));
  }, [bookings, services, staff, shopName, shopPhone, shopHours, staffAvailability, isLoaded]);
  ...
  const isStaffFull = (staffId: string) => Boolean(staffAvailability[staffId]);
  ...
  const fullyBookedStaff = staff.filter((member) => isStaffFull(member.id)).length;
  ...
  function toggleStaffFull(staffId: string) {
    setStaffAvailability((current) => ({ ...current, [staffId]: !current[staffId] }));
  }
  ...
  function resetRecords() {
    ...
    window.localStorage.removeItem(STAFF_AVAILABILITY_KEY);
  }
  ...
  function renderStats() {
    const cards = [
      ...
      ["Fully booked", fullyBookedStaff.toString(), "Staff unavailable today"],
    ];
    return (
            ...
            <div className="mt-6 space-y-4">
              {todaysSchedule.map((booking) => (
                <div key={booking.id} className="grid gap-4 rounded-2xl bg-cream p-5 md:grid-cols-[90px_1fr_auto] md:items-center">
                  <div className="w-fit rounded-full bg-white px-4 py-2 text-sm font-black text-rosewood">{booking.startTime}</div>
                  <div>
        ...
        <form onSubmit={addManualBooking} className={panelClass}>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            ...
            <select value={bookingStaffId} onChange={(event) => setBookingStaffId(event.target.value)} className="input-field">
              {staff.map((member) => <option key={member.id} value={member.id}>{member.name}{isStaffFull(member.id) ? " - Fully booked today" : ""}</option>)}
            </select>
        ...
        <div className="overflow-hidden rounded-[2rem] border border-rosewood/10 bg-white/82 shadow-sm">
          <div className="flex flex-col gap-5 border-b border-rosewood/10 p-6">
            ...
            <div className="grid gap-3 xl:grid-cols-[1fr_auto] xl:items-center">
              ...
              <div className="flex flex-wrap justify-start gap-2">
                {statusFilters.map((status) => <button key={status} type="button" onClick={() => setBookingStatusFilter(status)} className={`rounded-full px-4 py-2 text-xs font-black uppercase tracking-wide transition ${bookingStatusFilter === status ? "bg-rosewood text-white" : "bg-cream text-espresso/60 hover:bg-blush"}`}>{statusFilterLabel(status)}</button>)}
              </div>
            </div>
          </div>
            ...
            return (
              <div key={member.id} className="rounded-[1.5rem] bg-cream p-5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <AvailabilityBadge full={isStaffFull(member.id)} />
                </div>
                <button type="button" onClick={() => toggleStaffFull(member.id)} className={`mt-4 ${isStaffFull(member.id) ? primaryButton : secondaryButton}`}>{isStaffFull(member.id) ? "Mark available" : "Mark fully booked"}</button>
                <div className="mt-5 space-y-3">
                  {staffBookings.map((booking) => <div key={booking.id} className="rounded-2xl bg-white/80 p-4 text-sm"><p className="font-black">{booking.startTime} · {getServiceName(booking.serviceId)}</p><p className="mt-1 text-espresso/60">{booking.customerName}</p><div className="mt-3"><StatusBadge status={booking.status} /></div></div>)}
      ...
      <div className="space-y-8">
        <div className={panelClass}>
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-espresso/60">Mark a staff member as fully booked so customers know who is no longer available today.</p>
            </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          {filteredStaff.map((member) => {
            ...
            return (
              <div key={member.id} className="rounded-[1.5rem] border border-rosewood/10 bg-white/82 p-5 shadow-sm">
                ...
                <div className="mt-5 flex items-center justify-between gap-3"><AvailabilityBadge full={isStaffFull(member.id)} /><button type="button" onClick={() => toggleStaffFull(member.id)} className={isStaffFull(member.id) ? primaryButton : secondaryButton}>{isStaffFull(member.id) ? "Mark available" : "Mark full"}</button></div>
                <div className="mt-5 rounded-2xl bg-cream p-4">
                  <p className="text-xs font-black uppercase tracking-wide text-rosewood/60">Assigned services</p>
                  <div className="mt-3 flex flex-wrap gap-2">{assigned.slice(0, 4).map((serviceName) => <span key={serviceName} className="rounded-full bg-white px-3 py-1 text-xs font-black text-espresso/60">{serviceName}</span>)}{assigned.length > 4 ? <span className="rounded-full bg-blush px-3 py-1 text-xs font-black text-rosewood">+{assigned.length - 4} more</span> : null}</div>
                </div>
                <button type="button" onClick={() => removeStaff(member.id)} className="mt-5 rounded-full bg-red-100 px-5 py-3 text-sm font-black text-red-700 transition hover:-translate-y-0.5 hover:bg-red-200">Remove staff</button>
              </div>
            );
          })}
        </div>
    ...
    return (
      <div className={panelClass}>
        ...
        <div className="mt-6 grid gap-4 xl:grid-cols-2">
          {customers.map((customer) => <div key={customer.latest.customerPhone} className="rounded-[1.5rem] bg-cream p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-lg font-black">{customer.latest.customerName}</p><p className="mt-1 text-sm text-espresso/60">{customer.latest.customerPhone}</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-rosewood">{customer.bookings.length} visits</span></div><p className="mt-4 text-sm font-bold text-rosewood">Latest service: {getServiceName(customer.latest.serviceId)}</p><p className="mt-1 text-sm text-espresso/60">Total estimated spend: ₱{customer.spend.toLocaleString()}</p></div>)}
        </div>
      </div>
    );
  }
  ...
  return (
    <section className="min-h-screen bg-cream py-8 md:py-10">
      <aside className="hidden xl:flex fixed left-0 top-20 bottom-0 z-30 w-[292px] flex-col overflow-y-auto rounded-r-[2rem] bg-espresso p-6 text-white shadow-[0_25px_90px_rgba(42,23,20,0.22)]">
      ...
      <div className="salon-container max-w-[1500px] xl:ml-[316px] xl:w-[calc(100%-340px)]">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div><p className="text-sm font-black uppercase tracking-[0.26em] text-rosewood/70">Admin workspace</p><h1 className="mt-2 text-4xl font-black tracking-tight md:text-5xl">{activeTab}</h1></div>
          <div className="flex flex-wrap justify-start gap-2 xl:hidden">{tabs.map((item) => <button key={item} type="button" onClick={() => setActiveTab(item)} className={`rounded-full px-4 py-2 text-xs font-black ${activeTab === item ? "bg-rosewood text-white" : "bg-white text-rosewood"}`}>{item}</button>)}</div>
        </div>
